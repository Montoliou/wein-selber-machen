...
Inhaltsgleich mit `CLAUDE.md` daneben — Codex liest diese Datei, Claude jene. Die Regeln
reisen mit dem Projekt und gelten in allen vier Arbeitskontexten (Claude App, Claude Code,
...
2. **Arbeitet gerade jemand hier?** Prüfen, dann Marke setzen:
   ...
   `bash ~/Documents/Appentwicklung/Claude/MissionControl/automation-ops/arbeitsmarke.sh setzen "<Sitzungs-Name>" "/Users/Montolio/Documents/Appentwicklung/wein-selber-machen"`

## Nach dem Erzeugen (Gedächtnis-Konvention)

- Ergebnisse gehören nach `outputs/`; **zu jedem Artefakt `<name>` eine Begleitdatei
  `<name>.meta.md` daneben** (Vorlage: `AGENTS.md` im `outputs/`-Ordner einer MC-Domäne,
  z. B. `Claude/MissionControl/beratung-protokolle/outputs/AGENTS.md`).
